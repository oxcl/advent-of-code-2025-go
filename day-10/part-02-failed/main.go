...
func (machine Machine) Solve() (*Record, int, error) {
	...
	for machine.Queue.Size() > 0 && tries < 2_000_000 {
		...
		for index, action := range machine.Actions {
			...
			multipliersToTry := []int{1}
			if minDistanceToTarget > 1 {
				multipliersToTry = append(multipliersToTry, minDistanceToTarget)
			}
			if minDistanceToTarget > 4 {
				multipliersToTry = append(multipliersToTry, minDistanceToTarget/2)
			}
			for _, multiplier := range multipliersToTry {
